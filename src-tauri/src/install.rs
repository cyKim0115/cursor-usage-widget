...
const APP_FOLDER: &str = "CursorUsageWidget";
const EXE_NAME: &str = "cursor-usage-widget.exe";
const AUTOSTART_NAME: &str = "Cursor Usage Widget";
...
pub fn autostart_disable() -> Result<(), String> {
    ...
    for name in [AUTOSTART_NAME, "cursor-usage-widget"] {
...
(처음 한 번은 릴리스 빌드 후 %LOCALAPPDATA%\\CursorUsageWidget 에 설치됩니다.)\n\n\
개발 중이면 터미널에서:\nnpm run tauri dev",
        );
        std::process::exit(1);
    }
