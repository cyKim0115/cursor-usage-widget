...
description: >-
  Stops the running Cursor Usage Widget process, force-rebuilds the Tauri
...
# Cursor Usage Widget — rebuild & restart
...
1. `cursor-usage-widget` 프로세스 종료 (`Stop-RunningWidget`)
2. `npm run build:app` (VS Build Tools `vcvars64.bat` 경유, 필요 시 `npm install`)
3. `src-tauri/target/release/cursor-usage-widget.exe` → `%LOCALAPPDATA%\CursorUsageWidget\`
4. 설치 exe 실행

## 성공 확인

- `Get-Process -Name cursor-usage-widget` — 프로세스 1개 이상
...
- Tauri/UI/Rust 변경 후 **+커푸**까지 마쳤으면, 커밋·푸시 직후 이 스크립트로 설치본을 갱신한다.
